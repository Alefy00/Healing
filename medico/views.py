...
def finalizar_consulta(request, id_consulta):
    ...
    consulta.save()
    return redirect(f'/medicos/consulta_area_medico/{id_consulta}/')
#fazer o botão cancelar consulta!
...
def consulta_grafico(request):
    ...
    # Consultas do médico
    consultas = Consulta.objects.filter(data_aberta__user=dados_medico.user)

    # Dicionário para armazenar a contagem de consultas por dia
    consultas_por_dia = defaultdict(int)

    # Contagem das consultas por dia
    for consulta in consultas:
        data_consulta = consulta.data_aberta.data.date()
        consultas_por_dia[data_consulta] += 1

    # Converter o dicionário em uma lista de tuplas (data, contagem)
    consultas_por_dia = sorted(consultas_por_dia.items())

    # Passar os dados para a template
    return render(request, 'consulta_grafico.html', {'consultas_por_dia': consultas_por_dia})
